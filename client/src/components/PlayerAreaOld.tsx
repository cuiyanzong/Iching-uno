...
import { getPlayerScore } from "@/utils/permanentScores";
...
export default function PlayerArea({
  ...
  // 获取永久积分
  const playerId = player.isAI ? `ai_${player.id}` : 'human_player';
  const playerPermanentData = getPlayerScore(playerId);
  const permanentScore = playerPermanentData?.totalScore || 0;
